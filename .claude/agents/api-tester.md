...
Re-verify these open findings each run: draft routes (tracks, upload-url, process) skip ownership; a share cookie passes the global auth gate and unlocks GET /api/jobs and /api/workers; GET /api/meetings embeds every user's active jobs; _body_json has no size cap; /api/auth/login has no rate limit; translate `lang` is unvalidated.
