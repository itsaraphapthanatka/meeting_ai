...
Permission helpers in server.py: _is_owner, _may_read, _may_write, _may_write_job. New meeting sub-routes belong AFTER the permission block in _meeting() (around lines 887-900); the draft routes tracks/upload-url/process currently sit before it and skip ownership (open P0).
