...
```
เว็บ ──(คิว kind=bot)──► worker ที่มี Docker ──► บอท Chromium เข้าห้อง อัดเสียง
                                                        │
                          คลังการประชุม ◄── ถอดเสียง+แยกผู้พูด+สรุป (ท่อเดิมทั้งเส้น)
```
...
## บอทเข้าห้องประชุม (แนะนำสำหรับประชุมออนไลน์)

ส่ง "บอท" เข้าไปนั่งในห้องประชุมเหมือนผู้เข้าร่วมคนหนึ่ง อัดเสียงจากในห้องเลย —
**ไม่ต้องยุ่งกับลำโพง/BlackHole และแชร์หน้าจอได้ปกติ** (ไม่ชนกับ Zoom/Meet)

บอทรันใน **Docker** ทั้งหมด (Chromium + เสียงเสมือน) แยกจากเครื่องคุณ จึงไม่แตะเสียง/หน้าจอ host

**ขั้นที่ 1 — ล็อกอิน Google ให้บอทครั้งเดียว** (จำเป็น! ห้อง Google Workspace บล็อกคนที่ไม่ล็อกอิน)
```bash
./mai bot-login
```
จะเปิด **Screen Sharing (VNC)** ให้ล็อกอิน Google — แนะนำสร้าง**บัญชีเฉพาะสำหรับบอท**
(เช่น `notetaker@โดเมนคุณ`) ล็อกอินเสร็จกด Enter กลับมาที่ terminal เพื่อบันทึก session

**ขั้นที่ 2 — ส่งบอทเข้าห้อง**
```bash
./mai bot "https://meet.google.com/xxx-xxxx-xxx" --title "ประชุมทีม"
```
- บอทจะเข้าห้องในชื่อบัญชีที่ล็อกอินไว้
- **อย่าลืมกด "รับเข้าห้อง" (Admit)** ให้บอทในโปรแกรมประชุม
- กด `Ctrl+C` เมื่อจบ → บอทออกจากห้อง หยุดอัด แล้ว**ถอดเสียง+สรุปให้อัตโนมัติ**
- อยากอัดอย่างเดียวไม่สรุป: เพิ่ม `--no-process`

> session ที่ล็อกอินเก็บที่ `bot/profile/` (ไม่ commit) ล็อกอินครั้งเดียวใช้ได้ยาว
> จนกว่า Google จะหมดอายุ session — ถ้าบอทเข้าไม่ได้ให้รัน `./mai bot-login` ใหม่
...
## ตั้งค่าอัดเสียงประชุมออนไลน์ (วิธีอัดจากลำโพงเครื่อง)
