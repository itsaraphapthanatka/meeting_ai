...
# โหมดล็อกอินไม่เขียน /out เลย (ไม่ได้อัดเสียง) และ bot.login() ก็ไม่ mount มาให้ด้วย
# แต่ /out **มีอยู่** ในอิมเมจเพราะบรรทัด VOLUME ใน Dockerfile และเป็นของ root
# พอ BACKLOG #21 เปลี่ยนไปรันในนาม pwuser การตรวจนี้จึงล้มทุกครั้ง -> คอนเทนเนอร์ออกทันที
# ผลคือ `mai bot-login` พังสนิทเงียบ ๆ ตั้งแต่ #21 (BUG-068) และเพราะล็อกอินไม่เคยสำเร็จ
# บอทจึงเข้าห้องแบบไม่ระบุตัวตนตลอด ห้องที่ไม่รับ guest ก็ปฏิเสธ — อาการปลายทางคือ
# "ส่งบอทเข้าห้องแล้วไม่มีอะไรเกิดขึ้น" ซึ่งไล่ย้อนกลับมาถึงตรงนี้ยากมาก
if [ "$MODE" = "login" ]; then
    NEED_WRITE="/prof"
else
    NEED_WRITE="/out /prof"
fi
for d in $NEED_WRITE; do
    [ -d "$d" ] || continue
