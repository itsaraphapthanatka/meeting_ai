...
| 45 | `backend.storage()` picks S3/R2 whenever `S3_BUCKET` is set, ignoring `MEETING_AI_CLOUD` — `files` mode on a box with `.env` hands out presigned PUT URLs for the **production** bucket (hit for real during QA 2026-09-16) | devops-engineer + backend-dev | rotate R2 keys; split dev/test bucket; make `storage()` respect the mode or warn loudly |
...
| 49 | Before deploying the BUG-044 fix, audit production for pre-patch poisoned rows: `select id, status, attempts from meeting_ai.jobs where id ~ '[/\]';` — they now terminate on the next claim instead of looping, but the owner should know the count | owner + devops-engineer | read-only query; no DDL needed |
