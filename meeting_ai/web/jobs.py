...
# งานที่ค้างคิวนานกว่านี้ **และ** ไม่มีเครื่องไหนรับชนิดนั้นได้ ถือว่าไม่มีทางได้รัน (#87)
# เผื่อเวลาให้ worker รีสตาร์ต/ดีพลอยเสร็จก่อน — เท่ากับเพดานของ jobs_reap() ที่ใช้อยู่แล้ว
UNCLAIMABLE_MINUTES = 30
...
def claim(worker: str | None = None, kinds: list[str] | None = None) -> dict | None:
    ...
    if cloud:
        ...
        store.jobs_reap(30)
        # แล้วเก็บงานที่ไม่มีเครื่องไหนรับชนิดนั้นได้เลย — เกาะจังหวะเดียวกันเพราะ worker
        # เรียก claim สม่ำเสมออยู่แล้ว และ serverless ไม่มีโพรเซสค้างให้ตั้งเวลา (#87)
        store.jobs_fail_unclaimable(UNCLAIMABLE_MINUTES)
