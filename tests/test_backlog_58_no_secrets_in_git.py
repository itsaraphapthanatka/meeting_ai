...
**อัปเดต 2026-09-20**: เจ้าของหมุน `LLM_API_KEY` แล้ว ค่าใหม่หาไม่เจอในคอมมิตไหนเลย
เทสต์ข้อที่เคยยืนยันว่า "มีตัวเดียวที่หลุด" จึงกลับด้านเป็น "ต้องไม่มีเลย"
คีย์เก่ายังอยู่ใน history ตลอดไป แต่ใช้ไม่ได้แล้ว
"""
...
class TestWhatIsStillInHistory(unittest.TestCase):
    ...
    def test_no_live_secret_is_in_history(self):
        """หมุน `LLM_API_KEY` แล้ว 2026-09-20 — ค่าที่ใช้อยู่ตอนนี้ต้องไม่อยู่ใน history เลย.

        เดิมข้อนี้ยืนยันว่า **มี** `LLM_API_KEY` อยู่ใน history (คอมมิต `b6daf68`) เพราะ
        ตอนนั้นคีย์ที่หลุดยังเป็นคีย์ที่ใช้งานจริง · พอเจ้าของหมุนแล้ว ค่าใหม่หาไม่เจอ
        ในคอมมิตไหนเลย เทสต์จึงกลับด้าน: **ต้องว่าง**

        คีย์เก่ายังอยู่ใน history ตลอดไป (`rm` ลบไฟล์ ไม่ได้ลบ blob) แต่มันใช้ไม่ได้แล้ว
        จึงไม่ใช่ค่าลับที่ยังมีชีวิต — ถ้าจะลบ blob ออกจริงต้องเขียน history ใหม่ ซึ่งเป็น
        คนละงานและ runbook เขียนทางเลือกนั้นไว้แล้ว

        ถ้าข้อนี้แดงอีก แปลว่ามีค่าลับที่ **ใช้อยู่จริง** หลุดเข้า git — หมุนตัวนั้นทันที
        แล้วค่อยกลับมาดูเทสต์ ไม่ใช่กลับกัน
        """
        leaked = sorted(name for name, v in self.secrets.items() if self._commits_with(v))
        self.assertEqual(leaked, [],
                         "มีค่าลับที่ยังใช้อยู่โผล่ใน git history — หมุนตัวนั้นก่อน")
